...
stage1<-function(){
  ...
  fail<-function(){
    ...
    if(e$m[e$head[1],e$head[2]]==e$col_barrier){
  ...
  body<-function(){
    ...
    if(nrow(e$tail)>2) {
      e$tail<-rbind(e$tail,data.frame(x=e$lastx,y=e$lasty))
      e$m[e$tail[2,]$x,e$tail[2,]$y]<-e$col_path
      e$tail<-e$tail[-1,]
  ...
  drawTable<-function(){
    ...
    text(0.5,0.1,label=paste("You have eat",nrow(e$tail),"fruits!"),cex=2,col=2)
  }
  drawMatrix<-function(){
    idx<-which(e$m>0)
